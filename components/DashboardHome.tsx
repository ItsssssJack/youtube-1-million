...
const DashboardHome: React.FC<DashboardHomeProps> = ({ videos, ideas, onNavigate }) => {
  ...
  const getMultiplierStyle = (m?: number) => {
      ...
      if (m >= 2) return 'bg-[#14452F] text-white shadow-sm';
      if (m >= 1.2) return 'bg-emerald-100 text-emerald-800';
      ...
      <div className="flex justify-between items-end">
        ...
        <div className="flex gap-3">
            ...
            <button 
                ...
                className="bg-[#14452F] text-white px-6 py-2.5 rounded-full font-medium text-sm shadow-lg shadow-[#14452F]/20 hover:bg-[#0f3624] transition-colors flex items-center gap-2"
            >
                <PlayCircle size={16} />
                Add Idea
            </button>
            ...
            <div className="bg-white p-8 rounded-[2rem] shadow-sm">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        {ownVideosSorted.map(video => (
                            <div key={video.id} className="group cursor-pointer">
                                ...
                                <h4 className="font-bold text-gray-900 leading-snug line-clamp-2 group-hover:text-[#14452F] transition-colors">{video.title}</h4>
                ...
                {ownVideosSorted.length === 0 && (
                    <div className="py-12 text-center bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                        ...
                        <button onClick={() => onNavigate(ViewState.SETTINGS)} className="text-[#14452F] font-bold text-sm mt-2 hover:underline">
                            Sync Channel Data
                        </button>
                    </div>
                )}
            ...
            <div className="bg-white p-8 rounded-[2rem] shadow-sm">
                <div className="flex justify-between items-center mb-8">
                    <div>
                        ...
                        <p className="text-xs text-gray-400">Activity proxy based on recent uploads</p>
                    </div>
                </div>
                <div className="h-64 w-full">
                    ...
                    <BarChart data={data}>
                        ...
                        {data.map((entry, index) => (
                            <Cell key={`cell-${index}`} fill={index === 2 ? '#10b981' : '#14452F'} fillOpacity={index === 2 ? 0.6 : 1} />
                        ))}
                        </Bar>
                    </BarChart>
                    </ResponsiveContainer>
                ...
                {outliers.slice(0, 3).map((video) => (
                    <div key={video.id} className="flex gap-4 group cursor-pointer relative">
                        ...
                        <div className="flex-1 min-w-0">
                            <h4 className="text-sm font-bold text-gray-900 leading-snug line-clamp-2 group-hover:text-emerald-700 transition-colors">{video.title}</h4>
