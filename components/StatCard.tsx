...
const StatCard: React.FC<StatCardProps> = ({ label, value, change, trend, colorClass = "bg-[#14452F]", icon }) => {
  return (
    <div className={`rounded-[2rem] p-6 relative overflow-hidden group hover:shadow-lg transition-all duration-300 ${colorClass === 'bg-white' ? 'bg-white text-gray-900' : 'text-white ' + colorClass}`}>
      ...
      <div className="flex flex-col gap-1 relative z-10">
        ...
        <div className="flex items-center gap-2 mt-2">
           <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold ${
             trend === 'up' 
                ? (colorClass === 'bg-white' ? 'bg-emerald-100 text-emerald-700' : 'bg-white/20 text-white') 
      ...
      <div className={`absolute -bottom-6 -right-6 w-32 h-32 rounded-full opacity-10 ${colorClass === 'bg-white' ? 'bg-emerald-900' : 'bg-white'}`}></div>
    </div>
  );
};
